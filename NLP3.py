...
der = {('license', 'project'): ['http://purl.org/dc/terms/license', 'http://resources.samsung.com/projects/'],
       ('repository', 'project'): ['http://resources.samsung.com/git/repository', 'http://resources.samsung.com/projects/'],
       ...
       ('project', 'contributor'): ['http://purl.org/dc/terms/contributor', 'http://resources.samsung.com/contributors/'],
       ('contribute', 'repository'): ['http://purl.org/dc/terms/contributor', 'http://resources.samsung.com/repositories/'],
       ...
       ('subscriber', 'repository'): ['http://resources.samsung.com/git/subscribers', 'http://resources.samsung.com/repositories/'],
       ('star', 'repository'): ['http://resources.samsung.com/git/stars', 'http://resources.samsung.com/repositories/'],
       ('time', 'repository'): ['http://purl.org/dc/terms/created', 'http://resources.samsung.com/repositories/'],
       ('language', 'repository'): ['http://purl.org/dc/terms/language', 'http://resources.samsung.com/repositories/'],
       ('group', 'contributor'): ['http://resources.samsung.com/git/contributorGroup', 'http://resources.samsung.com/contributors/']}
doc = nlp(sent)
s = str(doc.sentences)
l = s.splitlines()
l = l[1:-1]
            ...
            if '"deprel":"obl"' in el or '"deprel":"nsubj"' in el or '"deprel":"conj"' in el or '"deprel":"root"' in el:
                ...
                for p in S:
                    if '"text":' in p:
                        p = p.replace('"text":', '')
                        p = p[1:-1]
                        if (lemma(query[n]), lemma(p)) in der.keys():
                            select += ' ?'
                            select += query[n]
                            write += '       '
                            rep = '<' + der[(lemma(query[n]), lemma(p))][0] + '> '
                            if qc != 0:
                                data1 = '<' + der[(lemma(query[n]), lemma(p))][1] + der1['ilia'] + '> '
                            else:
                                data1 = '<' + der[(lemma(query[n]), lemma(p))][1] + kc + '> '
                            write += data1
                            write += rep
                            write += '?' + query[n] + '.' + '\n'
                            G.append(query[n])
